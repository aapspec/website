...
import { Shield, Clipboard, Settings } from 'lucide-react';
...
export function Benefits() {
  ...
  const accentColors: Record<string, string> = {
    ...
    clipboard: 'from-violet-500 to-violet-600',
    settings: 'from-green-500 to-green-600'
  };

  const accentBorders: Record<string, string> = {
    shield: 'hover:border-blue-500/30',
    clipboard: 'hover:border-violet-500/30',
    settings: 'hover:border-green-500/30'
  };
  ...
  return (
    <Section id="benefits" background="muted" padding="lg" className="relative overflow-hidden">
      ...
      <div className="relative">
        ...
        <StaggerContainer className="grid grid-cols-1 md:grid-cols-3 gap-10 auto-rows-fr">
          {benefits.categories.map((category, index) => {
            const Icon = iconComponents[category.icon];
            const gradient = accentColors[category.icon];
            const borderHover = accentBorders[category.icon];
            return (
              <StaggerItem key={index} className="h-full">
                <div className={`group p-10 bg-white dark:bg-zinc-900 rounded-3xl border border-zinc-200 dark:border-zinc-800 shadow-xl h-full flex flex-col transition-all duration-300 hover:shadow-2xl ${borderHover}`}>
                  <div className="mb-10">
                    <div className={`inline-flex items-center justify-center w-20 h-20 rounded-2xl bg-gradient-to-br ${gradient} text-white shadow-lg mb-8 group-hover:scale-110 transition-transform duration-300`}>
